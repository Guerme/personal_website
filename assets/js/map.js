...
const STYLE = {
  ...
  mon_visited: new ol.style.Style({ image: new ol.style.Circle({ radius: 7, fill: new ol.style.Fill({ color: '#c97d4e' }), 
  stroke: new ol.style.Stroke({ color: '#0c0c0c', width: 1.5 }) }) }),
  mon_unvisited: new ol.style.Style({ image: new ol.style.Circle({ radius: 6, fill: new ol.style.Fill({ color: 'rgba(255,255,255,0.15)' }), 
  stroke: new ol.style.Stroke({ color: '#c97d4e', width: 1.5 }) }) }),
};
const HIGHLIGHT = {
...
function makeFeatures(data, type) {
  return data.map(d => {
    ...
    const blm = type === 'mon' ? getMonStyle(d.agency) : null;
    f.setStyle(blm ?? STYLE[`${type}_${d.visited ? 'visited' : 'unvisited'}`]);
    return f;
  });
}
...
const ICON_SIZE  = 20;  // px, must match icon width/height set in parkStyle — adjust here
...
function showPopup(pixel) {
  ...
  // Candidate positions: popup corner sits POPUP_GAP px away from the icon edge
  // Order of preference: top-left, top-right, bottom-left, bottom-right
  const candidates = [
    { left: iconLeft  - pw  - POPUP_GAP, top: iconTop - ph - POPUP_GAP },  // top-left
    { left: iconRight       + POPUP_GAP, top: iconTop - ph - POPUP_GAP },  // top-right
    { left: iconLeft  - pw  - POPUP_GAP, top: iconBottom   + POPUP_GAP },  // bottom-left
    { left: iconRight       + POPUP_GAP, top: iconBottom   + POPUP_GAP },  // bottom-right
  ];

  // Pick the first candidate that fits fully within the map
  const fits = ({ left, top }) =>
    left >= mapRect.left &&
...
function restoreStyle(feature) {
  const type = feature.get('type');
  const blm = type === 'mon' ? getMonStyle(feature.get('agency')) : null;
  feature.setStyle(blm ?? STYLE[`${type}_${feature.get('visited') ? 'visited' : 'unvisited'}`]);
}
...
map.on('singleclick', e => {
  ...
  if (feature) {
    ...
    requestAnimationFrame(() => showPopup(e.pixel));
  } else {
    closePopup();
  }
});
...
// Prevent legend interactions from reaching the map
legendEl.addEventListener('pointerdown', e => e.stopPropagation());
legendEl.addEventListener('click',       e => e.stopPropagation());

// Inject into OL viewport so it lives inside the map
map.getViewport().appendChild(legendEl);

// Position legend over the Pacific Ocean (between Hawaii and Alaska) on first render
...
// ── Ensure map renders when visible ──
const observer = new IntersectionObserver(entries => {
  entries.forEach(entry => {
    if (entry.isIntersecting) { map.updateSize(); observer.unobserve(entry.target); }
  });
}, { threshold: 0.1 });
